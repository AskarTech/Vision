            ...
            <header class="header-modern flex items-center justify-between px-6 py-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg" style="background: linear-gradient(135deg, #60a5fa 0%, #2563eb 100%);">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <button @click="openNotifications = !openNotifications" 
                                class="btn-modern btn-outline p-2" style="background: transparent; border: 1px solid var(--border-light);">
                        ...
                        <div x-show="openNotifications" x-cloak @click.outside="openNotifications=false"
                             class="absolute left-0 mt-2 w-80 rounded-xl shadow-xl border z-50" style="background: var(--bg-card); border-color: var(--border-light);">
                            ...
                            <div class="max-h-64 overflow-auto">
                                @forelse($recentAlerts as $alert)
                                    <div class="p-4 hover:bg-gray-50 border-b last:border-0" style="border-color: var(--border-light);">
                                        <div class="flex items-start justify-between gap-2">
                                            <div>
                                                <p class="text-sm font-semibold" style="color: var(--text-primary);">{{ $alert->code ?? 'تنبيه' }}</p>
                                                <p class="text-xs mt-1" style="color: var(--text-muted);">{{ $alert->report_reason ?? 'مراجعة مطلوبة' }}</p>
                                            </div>
                                            <x-ui.badge tone="info">{{ $alert->status ?? 'جديد' }}</x-ui.badge>
                                        </div>
                                    </div>
                                @empty
                                    <div class="p-4 text-center text-sm" style="color: var(--text-muted);">لا توجد تنبيهات حديثة</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        <button @click="openUser = !openUser" 
                                class="flex items-center gap-2 px-3 py-2 rounded-lg transition-colors" style="background: transparent;">
                            ...
                            <span class="text-sm font-medium hidden md:block" style="color: var(--text-secondary);">{{ auth()->user()->name ?? 'مسؤول' }}</span>
                        ...
                        <div x-show="openUser" x-cloak @click.outside="openUser=false"
                             class="absolute left-0 mt-2 w-48 rounded-xl shadow-lg border z-50" style="background: var(--bg-card); border-color: var(--border-light);">
                            <div class="p-3 border-b" style="border-color: var(--border-light);">
                                <p class="text-sm font-semibold" style="color: var(--text-primary);">{{ auth()->user()->name ?? 'مسؤول' }}</p>
                                <p class="text-xs" style="color: var(--text-muted);">{{ auth()->user()->email ?? '' }}</p>
                            </div>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-50" style="color: var(--text-secondary);">الإعدادات</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full text-left px-4 py-2 text-sm hover:bg-red-50" style="color: var(--color-danger-600);">
                                    تسجيل خروج
                                </button>
                            </form>
                        </div>
