<x-layouts.dashboard title="تعديل باقة" dashboardType="seller">
    ...
    <x-ui.panel class="rounded-[1.5rem]">
        <form method="POST" action="{{ route('seller.packages.update', $package) }}" class="grid gap-4 md:grid-cols-2">
            ...
            <label class="form-control md:col-span-2">
                ...
                <input name="name" value="{{ old('name', $package->name) }}" class="input input-bordered rounded-xl" required />
            </label>
            <label class="form-control">
                <span class="label-text">السعر</span>
                <input type="number" step="0.01" name="price" value="{{ old('price', $package->price) }}" class="input input-bordered rounded-xl" required />
            </label>
            <label class="form-control">
                <span class="label-text">الكمية</span>
                <input type="number" name="amount" value="{{ old('amount', $package->amount) }}" class="input input-bordered rounded-xl" />
            </label>
            <label class="form-control">
                <span class="label-text">الوحدة</span>
                <input name="unit" value="{{ old('unit', $package->unit) }}" class="input input-bordered rounded-xl" />
            </label>
            <label class="form-control">
            ...
            <label class="form-control md:col-span-2">
                ...
                <input name="category" value="{{ old('category', $package->category) }}" class="input input-bordered rounded-xl" required />
            </label>
            <label class="form-control md:col-span-2">
                <span class="label-text">Gradient</span>
                <input name="gradient" value="{{ old('gradient', $package->gradient) }}" class="input input-bordered rounded-xl" />
            </label>
            <label class="form-control md:col-span-2">
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                <a href="{{ route('seller.packages.index') }}" class="btn btn-ghost rounded-xl">رجوع</a>
            </div>
        </form>
    </x-ui.panel>
</x-layouts.dashboard>
