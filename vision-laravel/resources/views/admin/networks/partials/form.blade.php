<x-ui.panel>
    <form method="POST" action="{{ $action }}" class="grid gap-4 md:grid-cols-2">
        ...
        <label class="form-control">
            ...
            <input name="name" value="{{ old('name', $network?->name) }}" class="input input-bordered" required />
        </label>
        <label class="form-control">
            <span class="label-text">Slug</span>
            <input name="slug" value="{{ old('slug', $network?->slug) }}" class="input input-bordered" required />
        </label>
        <label class="form-control">
            <span class="label-text">Provider Code</span>
            <input name="provider_code" value="{{ old('provider_code', $network?->provider_code) }}" class="input input-bordered" />
        </label>
