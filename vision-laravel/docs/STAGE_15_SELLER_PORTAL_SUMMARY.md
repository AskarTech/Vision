...
- **Navigation:** `config/navigation.php` seller entries point at named routes (`seller.dashboard`, `seller.networks.index`, `seller.packages.index`, `seller.inventory.index`, `seller.orders.index`, `seller.sales.index`, `seller.inventory.create`, `seller.withdrawals.index`, `seller.wallet.index`, `seller.settings.index`).
